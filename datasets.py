...
def one_hot_df(df, df_cols, scaler=None, test=False, discrete=False):
...
def get_adults_dataframes(data_dir='data', discrete=False):
    column_names = ['age', 'workclass', 'fnlwgt', 'education', 'educational-num',\
                'marital-status', 'occupation', 'relationship', 'race',\
                'gender','capital-gain', 'capital-loss', 'hours-per-week', 'native-country','income']
    df_train = pd.read_csv(f'{data_dir}/adult.data', header=None, names=column_names)
    df_test = pd.read_csv(f'{data_dir}/adult.test', skiprows=1, names=column_names)
    
    scaler = StandardScaler()
    df_train = one_hot_df(df_train, df_train.select_dtypes('object'), scaler, discrete=discrete)\
    ...
    df_test = one_hot_df(df_test, df_test.select_dtypes('object'), scaler, test=True, discrete=discrete)\
                                    .drop(columns=['income_ <=50K.'])
